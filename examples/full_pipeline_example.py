...
def create_config(data_paths: dict, dirs: dict, sector: str) -> DSCIMConfig:
    ...
    print(f"  ----> Configured {len(sectors_config)} sectors")
    ...
    config = DSCIMConfig(
        ...
        econdata={
            "global_ssp": data_paths["economic"]
        },
        sectors=sectors_config,
        ...
        damage_function={
            "formula": sectors_config[sector]["formula"],
            "fit_type": "ols",
...
def run_pipeline(config: DSCIMConfig, sector: str, pulse_year: int, verbose: bool = False):
    ...
    print(f"  ----> Reduced damages dimensions: {dict(reduced_damages.sizes)}")
    print(f"  ----> Variables: {list(reduced_damages.data_vars)}")
    ...
    print(f"  ----> Coefficients dimensions: {dict(coefficients.sizes)}")
    print(f"  ----> Marginal damages dimensions: {dict(marginal_damages.sizes)}")
    ...
    for discount_idx, discount_config in enumerate(config.discounting):
        ...
        print(f"    ----> SCC calculated: dimensions {dict(scc.sizes)}")
...
def print_summary(results: dict, output_dir: str):
    ...
    print("\nGenerated outputs:")
    print(f"  -Reduced damages: {dict(results['reduced_damages'].sizes)}")
    print(f"  -Damage function coefficients: {dict(results['coefficients'].sizes)}")
    print(f"  -Marginal damages: {dict(results['marginal_damages'].sizes)}")

    print(f"\n  -SCC results ({len(results['scc'])} discounting methods):")
    for method, scc in results['scc'].items():
        print(f"    - {method}: {dict(scc.sizes)}")
